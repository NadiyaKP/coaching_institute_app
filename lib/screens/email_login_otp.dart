...
class EmailLoginOtpScreen extends StatefulWidget {
  const EmailLoginOtpScreen({super.key});

  @override
  State<EmailLoginOtpScreen> createState() => _EmailLoginOtpScreenState();
}

class _EmailLoginOtpScreenState extends State<EmailLoginOtpScreen> {
  ...
  // New variables for email login
  String email = '';
  String loginType = 'phone'; // Default to phone, will be overridden if email login
  ...
  void didChangeDependencies() {
    ...
    if (args != null) {
      ...
      if (args.containsKey('email') && args.containsKey('login_type')) {
        // Email login flow
        email = args['email'] ?? '';
        loginType = args['login_type'] ?? 'email';
        
        // Debug prints to verify email data
        _debugLog('EMAIL LOGIN OTP - ROUTE ARGUMENTS RECEIVED', {
          'email': email,
          'identifier': args['identifier'] ?? email,
          'login_type': loginType,
        });
      } else {
        // Phone login flow (existing logic)
        ...
        _debugLog('PHONE LOGIN OTP - ROUTE ARGUMENTS RECEIVED', {
  ...
  // Helper method to get the identifier (email or phone) for API calls
  ...
  // Helper method to get display text for the user
  ...
  // Helper method to determine navigation route based on profile completion
  void _navigateAfterSuccessfulLogin(Map<String, dynamic> responseData) {
    try {
      // Extract profile_completed status from response
      bool isProfileCompleted = false;
      
      if (responseData['profile_completed'] is bool) {
        isProfileCompleted = responseData['profile_completed'];
      } else if (responseData['profile_completed'] is String) {
        isProfileCompleted = responseData['profile_completed'].toString().toLowerCase() == 'true';
      }
      
      debugPrint('🧭 Profile completion status: $isProfileCompleted');
      
      // Prepare navigation arguments with any additional data from response
      final navigationArgs = {
        if (loginType == 'email') 'email': email else 'phone_number': phoneNumber,
        if (loginType == 'phone') 'country_code': countryCode,
        if (loginType == 'phone') 'mobile_number': mobileNumber,
        'verified': true,
        'login_success': true,
        'login_type': loginType,
      ...
      _debugLog('🧭 NAVIGATION DECISION', {
        'profile_completed': isProfileCompleted,
        'target_route': isProfileCompleted ? '/home' : '/profile_completion',
      ...
      if (isProfileCompleted) {
        ...
        // Profile is not completed - navigate to profile completion screen
      ...
      // Fallback to profile completion screen if there's an error
      debugPrint('🔄 Falling back to profile completion screen');
      Navigator.pushNamedAndRemoveUntil(
        context,
        '/profile_completion',
        (route) => false,
  ...
  Future<void> _verifyLoginOtp() async {
    ...
    debugPrint('Identifier: ${getIdentifier()}');
    ...
    try {
      ...
      // Prepare the request data with the correct identifier
      final requestData = {
        'identifier': getIdentifier(), // This will now correctly use email or phone
        'otp': otp,
      };
      ...
      _debugLog('🚀 LOGIN OTP API REQUEST', {
        ...
        'body': requestData,
        'login_type': loginType,
        'identifier_type': loginType == 'email' ? 'email' : 'phone',
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if (isSuccess) {
          ...
          if (mounted) {
            ...
            // Navigate based on profile completion status
            _navigateAfterSuccessfulLogin(responseData);
          }
        } else {
  ...
  Future<void> _resendLoginOtp() async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        final successMessage = responseData['message'] ?? 'Login OTP resent to ${getDisplayIdentifier()}';
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Login OTP resent to ${getDisplayIdentifier()}'),
            backgroundColor: AppColors.successGreen,
          ),
        );
          ...
          child: ConstrainedBox(
            ...
            child: Column(
              children: [
                ...
                Text(
                  getDisplayIdentifier().isNotEmpty 
                ...
                // Identifier Display (Email or Phone) - Reduced font size
                if (getDisplayIdentifier().isNotEmpty)
                  Text(
